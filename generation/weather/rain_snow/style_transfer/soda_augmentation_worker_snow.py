#!/usr/bin/env python3
"""
Worker script để augment SODA VOC dataset với SNOW effect
...
def parse_args():
    parser = argparse.ArgumentParser(description='Augment SODA VOC batch with SNOW')
...
def process_batch(image_ids, image_dir, output_dir, style_path, args, models=None):
    ...
    for idx, image_id in enumerate(batch_ids):
        ...
        try:
            ...
            # Process image with snow pipeline
            process_image(
...
def main():
    ...
    print("=" * 70)
    print("🌨️  SODA VOC SNOW AUGMENTATION WORKER")
    print("=" * 70)
    ...
    # Load VGG style transfer model
    print("   Loading VGG style transfer model...")
    vgg_dir = PROJECT_ROOT / 'VGG'
    vgg_checkpoint = resolve_vgg_checkpoint('snow', vgg_dir)
