...
Hỗ trợ multiple original arrow files (cho train dataset)
...
def parse_args():
    ...
    parser.add_argument('--original-arrow-dir', type=str, required=True,
                        help='Directory containing original arrow files')
    parser.add_argument('--original-arrow-pattern', type=str, default='*.arrow',
                        help='Glob pattern for original arrow files')
...
def load_multiple_arrow_files(arrow_dir, pattern):
    """Load samples from multiple arrow files"""
    all_samples = []
...
def main():
    ...
    print("="*70)
    print("SSIM-based Sample Filter (Multi-file Support)")
    print("="*70)
    print(f"Input directory: {args.input_dir}")
    print(f"Original dataset dir: {args.original_arrow_dir}")
    print(f"Original pattern: {args.original_arrow_pattern}")
    print(f"SSIM range: [{args.ssim_min}, {args.ssim_max}]")
    print(f"Output: {args.output}")
    print("="*70)
    
    # Load original dataset from multiple files
    print("\n📂 Loading original dataset...")
    original_samples = load_multiple_arrow_files(args.original_arrow_dir, args.original_arrow_pattern)
    if not original_samples:
        print("❌ Failed to load original dataset")
        sys.exit(1)
    print(f"✅ Loaded {len(original_samples)} total original samples")
