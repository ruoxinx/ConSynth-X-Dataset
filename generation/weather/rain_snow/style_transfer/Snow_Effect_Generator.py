...
class SnowEffectGenerator:
    ...
    def genSnowLayer(self, h=720, w=1280): #alpha, 
        ...
        # Generate at base resolution (720x1280) then scale
        # Using larger base if target is larger for better quality
        base_h, base_w = 720, 1280
        if h > 720 or w > 1280:
            scale = max(h / 720, w / 1280)
            base_h = min(int(720 * scale), h)
            base_w = min(int(1280 * scale), w)
        
        layer_small = self._snow_layer_gen.genSnowMultiLayer(h=base_h, 
                                                             w=base_w, 
                                                             blur_angle=blur_angle,
                                                             intensity="small", 
                                                             num_itr=num_itr_small)#small
        
        layer_large = self._snow_layer_gen.genSnowMultiLayer(h=base_h, 
                                                             w=base_w, 
                                                             blur_angle=blur_angle,
                                                             intensity="large", 
        ...
        if h!=hl or w!=wl:
            layer = np.asarray(Image.fromarray(layer).resize((w, h), Image.LANCZOS))
