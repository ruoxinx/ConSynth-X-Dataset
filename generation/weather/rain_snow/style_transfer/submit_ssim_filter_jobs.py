#!/usr/bin/env python3
"""
Submit SSIM filtering jobs to SLURM (no GPU required)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Submit SSIM filter jobs')
    
    # Input/Output
    parser.add_argument('--input-root', type=str, default='output/construction_site_test',
                        help='Root directory containing style_* folders')
    parser.add_argument('--original-arrow', type=str, 
                        default='LouisChen15___construction_site/construction_site-test.arrow',
                        help='Original dataset arrow file')
    parser.add_argument('--output-root', type=str, default='output/construction_site_test/filtered',
    ...
    parser.add_argument('--time', type=str, default='02:00:00', help='Job time limit')
...
def main():
    ...
    print("="*70)
    print("SSIM Filter Job Submission")
    print("="*70)
    print(f"Input root: {args.input_root}")
    print(f"Original arrow: {args.original_arrow}")
    print(f"Output root: {args.output_root}")
    print(f"Partition: {args.partition} (no GPU)")
    print("="*70)
    
    # Verify original arrow exists
    original_arrow = Path(args.original_arrow)
    if not original_arrow.exists():
        print(f"❌ Original arrow file not found: {original_arrow}")
        return
    ...
    # Create directories
    jobs_dir = Path('jobs/ssim_filter')
    logs_dir = Path('logs/ssim_filter')
    ...
    for style_dir in styles:
        ...
        # Job name
        job_name = f"ssim_{style_name}"
        
        # Worker command
        worker_cmd = (
            f"python generation/weather/ssim_filter_worker.py "
            f"--input-dir {style_dir} "
            f"--original-arrow {original_arrow} "
