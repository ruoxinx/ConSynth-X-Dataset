#!/usr/bin/env python3
"""
Worker script để augment dataset và output .arrow file
...
def parse_args():
    parser = argparse.ArgumentParser(description='Augment dataset batch and output .arrow')
...
def process_batch(table, start_idx, end_idx, style_path, args, models=None):
    ...
    for idx in range(start_idx, end_idx):
        try:
            ...
            try:
                # Process image with snow pipeline (using pre-loaded models)
                process_image(
...
def main():
    ...
    print("=" * 70)
    print("🔧 ARROW AUGMENTATION WORKER")
    print("=" * 70)
    ...
    vgg_ckpt = resolve_vgg_checkpoint('snow', vgg_dir)
