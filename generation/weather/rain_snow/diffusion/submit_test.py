#!/usr/bin/env python3
"""
Submit SLURM jobs for diffusion augmentation (IP2P + physics + filter) — TEST set.

Usage:
    python submit_test.py                    # submit all
    python submit_test.py --weather rain      # rain only
    python submit_test.py --dry-run           # preview
"""
...
def parse_args():
    ...
    parser.add_argument('--output-dir', type=str,
                        default='augmentation_data/construction_site/rain_snow_v4')
...
def main():
    args = parse_args()
    work_dir = Path.cwd()
    input_file = work_dir / args.input

    total = count_arrow(input_file)
    start = args.start
    ...
    jobs_dir = work_dir / 'generation' / 'weather' / 'diffusion' / 'jobs' / 'test'
    logs_dir = work_dir / 'generation' / 'weather' / 'diffusion' / 'logs' / 'test'
    ...
    for weather in weather_types:
        ...
        for batch_idx in range(num_batches):
            ...
            job_name = f"v4_{weather}_{b_start}-{b_end}_{timestamp}"
...
cd {work_dir}
echo "Starting {weather} batch {b_start}-{b_end} at $(date)"
nvidia-smi
{cmd}
echo "Done at $(date)"
"""
    ...
    with open(job_list, 'w') as f:
        f.write(f"# V4 batch jobs - {timestamp}\n")
        for j in all_jobs:
    ...
    if not args.dry_run:
        ...
        if ids:
            print(f"\nMonitor: squeue -u $USER | grep v4_")
