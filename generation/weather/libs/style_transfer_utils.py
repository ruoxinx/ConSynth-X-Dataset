...
def style_content_image_loader(content_path, style_path, max_size=None):
    """Load content and style images.
    
    Args:
        content_path: Path to content image
        style_path: Path to style image  
        max_size: Maximum dimension (width or height). None = keep original size.
                  Set to smaller value (e.g. 640, 800) for faster processing.
    """
    ...
    if max_size is not None:
        scale = min(max_size / wc, max_size / hc, 1.0)  # Don't upscale
        if scale < 1.0:
            wc_new, hc_new = int(wc * scale), int(hc * scale)
            content_img = content_img.resize((wc_new, hc_new), Image.LANCZOS)
            wc, hc = wc_new, hc_new
    
    style_img = Image.open(style_path).convert('RGB')
    ws, hs = style_img.size
    
    # Scale style to match content dimensions
    scale_w = wc / ws
    ws_new = wc
    hs_new = int(hs * scale_w)

    if hs_new < hc:
        hs_new = hc

    style_img = style_img.resize((ws_new, hs_new), Image.LANCZOS)

    if hs_new > hc:
        top = int((hs_new - hc) * 0.5)
        bottom = top + hc
