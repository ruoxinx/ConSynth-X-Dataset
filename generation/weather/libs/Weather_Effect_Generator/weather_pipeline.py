#!/usr/bin/env python3
"""
Weather Effect Pipeline - Unified Rain & Snow Pipeline
Combines Style Transfer + Weather Particles (Rain or Snow)

Pipeline: Input Image → Style Transfer (Weather tone) → Weather Effect (Particles) → Output

Usage:
    # Rain effect
    python weather_pipeline.py --input image.jpg --style rain_style.jpg --weather-type rain
    
    # Snow effect  
    python weather_pipeline.py --input image.jpg --style snow_style.jpg --weather-type snow
    
    # With custom parameters
    python weather_pipeline.py --input image.jpg --style style.jpg \\
        --weather-type rain --intensity quiet_night --steps 100
"""
...
# ====== UNIFIED INTENSITY CONFIG ======
# Updated quiet_night with new weights: visibility_ratio=(0.005, 0.05), darkness={0: 1.0, 1: 0.45, 2: 0.25, 3: 0.1}
INTENSITY_CONFIG = {
    'none': {
    ...
    'light': {
        ...
        'darkness': {0: 1.0, 1: 0.96, 2: 0.92, 3: 0.88},
    },
    'medium': {
        'visibility_ratio': (0.3, 0.6),
        'darkness': {0: 1.0, 1: 0.9, 2: 0.8, 3: 0.7},
    },
    'heavy': {
        'visibility_ratio': (0.1, 0.3),
        'darkness': {0: 1.0, 1: 0.75, 2: 0.6, 3: 0.5},
    },
    'extreme': {
        'visibility_ratio': (0.05, 0.15),
        'darkness': {0: 1.0, 1: 0.6, 2: 0.45, 3: 0.3},
    },
    'quiet_night': {
...
def get_weather_generator(weather_type: str):
    """Get the appropriate weather effect generator."""
    if weather_type == 'rain':
        return RainEffectGenerator()
    elif weather_type == 'snow':
        return SnowEffectGenerator()
    else:
        raise ValueError(f"Unknown weather type: {weather_type}. Use 'rain' or 'snow'.")


def get_weather_emoji(weather_type: str) -> str:
    """Get emoji for weather type."""
    return '🌧️' if weather_type == 'rain' else '❄️'
...
def process_image(
    ...
    """
    Process a single image with weather pipeline.
    ...
    Args:
        ...
        weather_type: Type of weather effect ('rain' or 'snow')
        steps: Style transfer steps
        style_weight: Style loss weight
        content_weight: Content loss weight
        intensity: Weather intensity (light/medium/heavy/extreme/quiet_night)
        use_fake_depth: Use fake depth instead of MiDaS
        weather_mode: VGG checkpoint mode (auto = same as weather_type)
    ...
    # Auto-detect weather mode for VGG checkpoint
    if weather_mode == 'auto':
        weather_mode = weather_type
    ...
    if style_path.exists():
        ...
        if vgg_model is None:
            ...
            vgg_ckpt = resolve_vgg_checkpoint(weather_mode, vgg_dir)
    ...
    weather_gen = get_weather_generator(weather_type)
    weather_gen._weather2visibility = (vis_min, vis_max)
    weather_gen._illumination2darkness = cfg['darkness']
    
    # Apply weather effect
    weather_result = weather_gen.genEffect(str(temp_image_path), str(temp_depth_path))
    current_image = Image.fromarray(weather_result)
    
    # ===== CRITICAL: Free ALL memory =====
    del weather_gen, weather_result, depth_map, img_array
    gc.collect()
    if DEVICE == 'cuda':
        torch.cuda.empty_cache()
        torch.cuda.reset_peak_memory_stats()
...
def main():
    parser = argparse.ArgumentParser(
        description='Weather Effect Pipeline: Style Transfer + Weather Particles (Rain/Snow)',
...
Examples:
  ...
  # With custom parameters
  python weather_pipeline.py --input image.jpg --style style.jpg \\
    --weather-type rain --steps 100 --style-weight 200000 --intensity quiet_night
  
  # Skip style transfer (only add weather particles)
  python weather_pipeline.py --input image.jpg --weather-type snow --skip-style-transfer --intensity medium
  
  # Skip weather particles (only style transfer)  
  python weather_pipeline.py --input image.jpg --style style.jpg --weather-type rain --skip-weather-particles
        """
    )
    
    # Input/Output
    parser.add_argument('--input', '-i', required=True, help='Path to input image')
    parser.add_argument('--style', '-s', default=None, help='Path to style image')
    parser.add_argument('--output', '-o', default=None, help='Output file path (auto-generated if not set)')
    
    # Weather type (REQUIRED)
    parser.add_argument('--weather-type', '-t', required=True, choices=['rain', 'snow'],
                       help='Type of weather effect: rain or snow')
    
    # Style Transfer parameters
    parser.add_argument('--weather-mode', '-w', default='auto', choices=['snow', 'rain', 'fog', 'imagenet', 'auto'],
                       help='Weather mode for VGG checkpoint (default: auto = same as weather-type)')
    ...
    # Weather Particles parameters
    parser.add_argument('--intensity', default='medium', 
                       choices=['none', 'light', 'medium', 'heavy', 'extreme', 'quiet_night'],
                       help='Weather particle intensity (default: medium). Use "none" to skip weather particles.')
    ...
    parser.add_argument('--skip-style-transfer', action='store_true',
                       help='Skip style transfer, only add weather particles')
    parser.add_argument('--skip-weather-particles', action='store_true',
                       help='Skip weather particles, only do style transfer')
    ...
    # Auto-detect weather mode
    weather_mode = args.weather_type if args.weather_mode == 'auto' else args.weather_mode
    emoji = get_weather_emoji(args.weather_type)
    weather_name = args.weather_type.upper()
    
    print('=' * 70)
    print(f'{emoji}  {weather_name} EFFECT PIPELINE')
    print('    Style Transfer + Weather Particles')
    print('=' * 70)
    ...
    if args.output:
        ...
        output_path = output_dir / f'{input_path.stem}-{args.weather_type}-pipeline.jpg'
    ...
    if not args.skip_style_transfer:
        print()
        print('─' * 50)
        print(f'STEP 1: Style Transfer ({weather_name} tone)')
        print('─' * 50)
        ...
        if args.style is None:
            ...
            if args.weather_type == 'rain':
                ...
                style_dir = PROJECT_ROOT.parent / 'style'
                pattern = '*snow*.jpg'
            
            style_candidates = list(style_dir.glob(pattern)) if style_dir.exists() else []
            if style_candidates:
                style_path = style_candidates[0]
                print(f'  ✓ Auto-found style image: {style_path.name}')
            else:
                raise FileNotFoundError(f'No style image provided. Use --style <path> or place {args.weather_type} style in appropriate folder')
        else:
    ...
    # STEP 2: WEATHER PARTICLES
    # ========================================
    if not args.skip_weather_particles:
        print()
        print('─' * 50)
        print(f'STEP 2: {weather_name} Particles (Weather Effect)')
        print('─' * 50)
        
        # Save current image temporarily for weather generator (use PNG to avoid quality loss)
        ...
        # Create weather generator with custom parameters
        weather_gen = get_weather_generator(args.weather_type)
        weather_gen._weather2visibility = (vis_min, vis_max)
        weather_gen._illumination2darkness = cfg['darkness']
        
        # Apply weather effect
        print(f'  Applying {args.weather_type} particles...')
        weather_result = weather_gen.genEffect(str(temp_image_path), str(temp_depth_path))
        current_image = Image.fromarray(weather_result)
        print(f'  ✓ {weather_name} particles added!')
    ...
    print()
    print('=' * 70)
    print(f'{emoji}  Pipeline complete!')
    print('=' * 70)
