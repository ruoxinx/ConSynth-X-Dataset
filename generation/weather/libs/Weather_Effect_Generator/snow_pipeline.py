#!/usr/bin/env python3
"""
Snow Effect Pipeline - Combines Style Transfer + Snow Particles
Pipeline: Input Image → Style Transfer (Snow tone) → Weather Effect (Snow particles) → Output

Usage:
    python snow_pipeline.py --input image.jpg --style snow_style.jpg
    python snow_pipeline.py --input image.jpg --style snow_style.jpg --intensity heavy --steps 100
"""
...
def process_image(
    ...
    """
    Process a single image with snow pipeline.
    ...
    Args:
        ...
        intensity: Snow intensity (light/medium/heavy/extreme/quiet_night)
    ...
    # Create snow generator
    snow_gen = SnowEffectGenerator()
    snow_gen._weather2visibility = (vis_min, vis_max)
    snow_gen._illumination2darkness = cfg['darkness']
    
    # Apply snow effect
    snow_result = snow_gen.genEffect(str(temp_image_path), str(temp_depth_path))
    current_image = Image.fromarray(snow_result)
    
    # ===== CRITICAL: Free ALL memory =====
    del snow_gen, snow_result, depth_map, img_array
    import gc
    gc.collect()
    if DEVICE == 'cuda':
        torch.cuda.empty_cache()
...
def main():
    parser = argparse.ArgumentParser(
        description='Snow Effect Pipeline: Style Transfer + Snow Particles',
...
Examples:
  ...
  # Skip style transfer (only add snow particles)
  python snow_pipeline.py --input image.jpg --skip-style-transfer --intensity medium
  
  # Skip snow particles (only style transfer)  
  python snow_pipeline.py --input image.jpg --style snow_style.jpg --skip-snow-particles
        """
    )
    
    # Input/Output
    parser.add_argument('--input', '-i', required=True, help='Path to input image')
    parser.add_argument('--style', '-s', default=None, help='Path to style image (snow reference)')
    parser.add_argument('--output', '-o', default=None, help='Output file path (auto-generated if not set)')
    
    # Style Transfer parameters
    parser.add_argument('--weather', '-w', default='snow', choices=['snow', 'rain', 'fog', 'imagenet'],
                       help='Weather mode for VGG checkpoint (default: snow)')
    ...
    # Snow Particles parameters
    parser.add_argument('--intensity', default='medium', 
                       choices=['none', 'light', 'medium', 'heavy', 'extreme', 'quiet_night'],
                       help='Snow particle intensity (default: medium). Use "none" to skip weather particles.')
    ...
    parser.add_argument('--skip-style-transfer', action='store_true',
                       help='Skip style transfer, only add snow particles')
    parser.add_argument('--skip-snow-particles', action='store_true',
                       help='Skip snow particles, only do style transfer')
    ...
    print('=' * 70)
    print('❄️  SNOW EFFECT PIPELINE')
    print('    Style Transfer + Snow Particles')
    print('=' * 70)
    ...
    if args.output:
        ...
        output_path = output_dir / f'{input_path.stem}-snow-pipeline.jpg'
    ...
    # ========================================
    # STEP 1: STYLE TRANSFER (Snow tone)
    # ========================================
    if not args.skip_style_transfer:
        print()
        print('─' * 50)
        print('STEP 1: Style Transfer (Snow tone)')
        print('─' * 50)
        
        if args.style is None:
            # Try to find style image
            style_dir = PROJECT_ROOT.parent / 'style'
            style_candidates = list(style_dir.glob('*snow*.jpg')) if style_dir.exists() else []
            if style_candidates:
                style_path = style_candidates[0]
                print(f'  ✓ Auto-found style image: {style_path.name}')
            else:
                raise FileNotFoundError('No style image provided. Use --style <path> or place snow style in style/ folder')
        else:
    ...
    # ========================================
    # STEP 2: SNOW PARTICLES (Weather Effect)
    # ========================================
    if not args.skip_snow_particles:
        print()
        print('─' * 50)
        print('STEP 2: Snow Particles (Weather Effect)')
        print('─' * 50)
        
        # Save current image temporarily for snow generator (use PNG to avoid quality loss)
        ...
        # Create snow generator with custom parameters
        snow_gen = SnowEffectGenerator()
        snow_gen._weather2visibility = (vis_min, vis_max)
        snow_gen._illumination2darkness = cfg['darkness']
        
        # Apply snow effect
        print(f'  Applying snow particles...')
        snow_result = snow_gen.genEffect(str(temp_image_path), str(temp_depth_path))
        current_image = Image.fromarray(snow_result)
        print('  ✓ Snow particles added!')
