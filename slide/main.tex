...
\begin{itemize}
    \item \textcolor{warnred}{Night} and \textcolor{warnred}{small} degrade all models
    \item F1 drops 15--60\% from original to worst condition
    \item \textcolor{boxblue}{Weather} has moderate impact
    \item IoU drops more severely than classification metrics
\end{itemize}

\vspace{0.5em}
\textbf{Model Comparison:}
\begin{itemize}
    \item \textbf{Gemma-3-27B}: Highest F1 (13.3\%), most robust across conditions
    \item \textbf{InternVL2.5-26B}: High recall (77--84\%) but low precision
    \item \textbf{Qwen3-VL-8B}: Best bbox IoU (29\%), conservative
    \item \textbf{GPT-4o-mini}: $\sim$50\% error rate
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Per-Rule Analysis:}

\vspace{0.3em}
{\footnotesize
\begin{tabular}{lcc}
\toprule
\textbf{Rule} & \textbf{Best F1} & \textbf{Challenge} \\
\midrule
Rule 1 (PPE) & 0.244 & Most common\\
Rule 2 (Harness) & 0.200 & Rare violations \\
Rule 3 (Edge) & 0.267 & Spatial reasoning \\
Rule 4 (Excavator) & 0.129 & Hardest overall \\
\bottomrule
\end{tabular}
}

\vspace{0.6em}
\textbf{Key finding:}\\[0.2em]
{\footnotesize All models struggle with construction safety VQA. Even the best model achieves only $\sim$0.30 IoU, suggesting current VLMs lack the spatial and domain understanding needed for reliable safety inspection.}
\end{column}
\end{columns}
\end{frame}
...
\textbf{Key findings:}
\begin{itemize}
    \item \textbf{Phi-4} and \textbf{InternVL2.5-26B} lead on text metrics (BLEU, BERT-F1)
    \item \textbf{CLIPScore} high across all models (0.74--0.83) -- descriptions are semantically relevant
    \item \textbf{LLaVA-1.5-7B} weakest across all metrics
    \item Larger models $\neq$ always better (Phi-4 6B $\approx$ InternVL 26B)
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    \item[\textcolor{okgreen}{$\checkmark$}] 3 augmentation pipelines (weather, night, outpainting)
    \item[\textcolor{okgreen}{$\checkmark$}] 85,613 total augmented images
    \item[\textcolor{okgreen}{$\checkmark$}] VLM benchmark framework (11 models)
    \item[\textcolor{okgreen}{$\checkmark$}] VQA Safety evaluation (5 models $\times$ 4 conditions)
    \item[\textcolor{okgreen}{$\checkmark$}] YOLOv8 training configs (5+3 experiments)
    \item[\textcolor{okgreen}{$\checkmark$}] Taxonomy of 40+ conditions
    \item[\textcolor{okgreen}{$\checkmark$}] Dual format support (Arrow + VOC)
    \item[\textcolor{okgreen}{$\checkmark$}] Repository consolidated into ConSynth-X
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{\textcolor{infobl}{In Progress / Next:}}
\begin{itemize}
    \item[\textcolor{infobl}{$\circ$}] Run remaining VLMs (Qwen 32B/72B, Gemma-3)
    \item[\textcolor{infobl}{$\circ$}] Description task evaluation (BERTScore, CLIPScore)
